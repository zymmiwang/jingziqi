...
namespace supping_qidong
{
    ...
    {RegistryKey regRootKey;
        ...
        public Form1()
        {
            ...
            RegistryKey Zane = Key.OpenSubKey(@"SOFTWARE\jingziqi\Settings", true);
            

            //string pwd = Zane.GetValue("suoping-pw").ToString();


            
            if (Zane != null)
            {
                string b = Zane.GetValue("suoping-danci").ToString();
                string b2 = Zane.GetValue("suoping-kaiji").ToString();
                if (b2=="no")
                {
               ...
               if (b == "yes"&&b2=="yes")
                {
                    Zane.SetValue("suoping-danci", "no");
                    Zane.SetValue("suoping-kaiji", "no");
                    RegistryKey a = Key.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true);
                    a.DeleteValue("suoping-kajiziqi");
                }
                else if(b2 == "yes"&&b=="no")
                {
                    RegistryKey a = Key.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true);
                ...
                pw = Zane.GetValue("suoping-pw").ToString();

            }
            else
            {
                    MessageBox.Show("本程序遭到修改，即将关闭。");
                    System.Environment.Exit(0);
            }
            if (pw == "")
            {
                ...
                Hook_Clear();
                CMD c2 = new CMD();
                string a2 = c2.cmd("pssuspend.exe winlogon.exe -r");
            ...
            di = Zane.GetValue("suoping-dir").ToString();
            ...
            if (di == "")
            {
                ...
                Hook_Clear();
                CMD c3 = new CMD();
                string a3 = c3.cmd("pssuspend.exe winlogon.exe -r");
                //SendMessageW(this.Handle, WM_APPCOMMAND, this.Handle, (IntPtr)APPCOMMAND_VOLUME_MUTE);
                SleepCtr(false);
                Hook_Clear();
                //string a2 = c.cmd("lanpingqingli.exe");
